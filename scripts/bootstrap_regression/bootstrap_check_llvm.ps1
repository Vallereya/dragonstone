...
New-Item -ItemType Directory -Force -Path ".\bin\dev\bootstrap\build" | Out-Null

function Run-Example([string]$file) {
    Write-Host ""
    Write-Host "============================================================"
    Write-Host "FILE: $file"
    Write-Host "CMD : .\bin\build\dragonstone.exe run .\bootstrap\bin\main.ds build-run --target llvm --output .\bin\dev\bootstrap\build $file"
    & .\bin\build\dragonstone.exe run .\bootstrap\bin\main.ds build-run --target llvm --output .\bin\dev\bootstrap\build $file
}
