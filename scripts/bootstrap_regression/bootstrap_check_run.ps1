...
function Run-Backend([string]$file, [string]$backend) {
    ...
    if ([string]::IsNullOrWhiteSpace($backend)) {
        ...
        Write-Host "CMD    : .\bin\build\dragonstone.exe run .\bootstrap\bin\main.ds run --backend $backend $file"
        & .\bin\build\dragonstone.exe run .\bootstrap\bin\main.ds run --backend $backend $file
    }
}
