...
function Run-Example([string]$file) {
    ...
    Write-Host "CMD : .\bin\build\dragonstone.exe run .\bootstrap\bin\main.ds build-run --target c --output .\bin\dev\bootstrap\build $file"
    & .\bin\build\dragonstone.exe run .\bootstrap\bin\main.ds build-run --target c --output .\bin\dev\build $file
}
