#!/usr/bin/env bash
set -euo pipefail
mkdir -p ./bin/dev/bootstrap/build
...
run() {
    ...
    echo "============================================================"
    echo "FILE: $file"
    echo "CMD : ./bin/dragonstone.sh run ./bootstrap/bin/main.ds build-run --target llvm --output ./bin/dev/bootstrap/build $file"
    ./bin/dragonstone.sh run ./bootstrap/bin/main.ds build-run --target llvm --output ./bin/dev/bootstrap/build "$file"
}
