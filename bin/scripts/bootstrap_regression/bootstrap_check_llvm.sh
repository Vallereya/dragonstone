...
run() {
    ...
    echo "------------------------------------------------------------"
    echo "FILE: $file"
    echo "CMD : ./bin/dragonstone.sh run $DS_MAIN build-run --target llvm --output $DS_DEV_BUILD $file"
    ./bin/dragonstone.sh run "$DS_MAIN" build-run --target llvm --output "$DS_DEV_BUILD" "$file"
}
