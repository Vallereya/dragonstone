...
run_backend() {
    ...
    if [[ -z "$backend" ]]; then
        ...
        echo "CMD    : ./bin/dragonstone.sh run $DS_MAIN run $file"
        ./bin/dragonstone.sh run "$DS_MAIN" run "$file"
    else
        echo "BACKEND: $backend"
        echo "CMD    : ./bin/dragonstone.sh run $DS_MAIN run --backend $backend $file"
        ./bin/dragonstone.sh run "$DS_MAIN" run --backend "$backend" "$file"
    fi
}
