...
function Run-Backend([string]$file, [string]$backend) {
    ...
    if ([string]::IsNullOrWhiteSpace($backend)) {
        ...
        Write-Host "CMD    : $DsExe run $DsMain run $file"
        & $DsExe run $DsMain run $file
    } else {
        Write-Host "BACKEND: $backend"
        Write-Host "CMD    : $DsExe run $DsMain run --backend $backend $file"
        & $DsExe run $DsMain run --backend $backend $file
    }
}
