...
module Dragonstone
    module Runtime
        ...
        class Engine
            ...
            def initialize(@resolver : ModuleResolver, @log_to_stdout : Bool = false)
                @unit_cache = {} of String => Unit
            end

            def compile_or_eval(ast : AST::Program, path : String) : Unit
                unit = Unit.new(path, Interpreter.new(log_to_stdout: @log_to_stdout))
