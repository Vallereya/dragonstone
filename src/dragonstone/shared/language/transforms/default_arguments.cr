    ...
    module Language
        module Transforms
            module DefaultArguments
                ...
                record FunctionDefaults, parameters : Array(AST::TypedParameter), required_count : Int32
                ...
                private def rewrite_node(node : AST::Node, defaults : Hash(String, FunctionDefaults)) : AST::Node
                    ...
                    when AST::ArrayLiteral
                        AST::ArrayLiteral.new(node.elements.map { |e| rewrite_node(e, defaults) }, node.element_type, location: node.location)
                    ...
                    when AST::IndexAssignment
                        AST::IndexAssignment.new(rewrite_node(node.object, defaults), rewrite_node(node.index, defaults), rewrite_node(node.value, defaults), operator: node.operator, nil_safe: node.nil_safe, location: node.location)
                    when AST::AttributeAssignment
                        AST::AttributeAssignment.new(rewrite_node(node.receiver, defaults), node.name, rewrite_node(node.value, defaults), operator: node.operator, location: node.location)
                    when AST::MethodCall
                    ...
                    when AST::IfStatement
                        ...
                        elsif_blocks = node.elsif_blocks.map do |clause|
                            AST::ElsifClause.new(rewrite_node(clause.condition, defaults), clause.block.map { |n| rewrite_node(n, defaults) }, location: clause.location)
                        end
                    ...
                    when AST::CaseStatement
                        ...
                        whens = node.when_clauses.map do |clause|
                            AST::WhenClause.new(clause.conditions.map { |c| rewrite_node(c, defaults) }, clause.block.map { |n| rewrite_node(n, defaults) }, location: clause.location)
                        end
                        else_block = node.else_block.try { |block| block.map { |n| rewrite_node(n, defaults) } }
                    ...
                    when AST::BeginExpression
                        ...
                        rescue_clauses = node.rescue_clauses.map do |clause|
                            AST::RescueClause.new(clause.exceptions, clause.exception_variable, clause.body.map { |n| rewrite_node(n, defaults) }, location: clause.location)
                        end
                    ...
                    when AST::FunctionDef
                        ...
                        AST::FunctionDef.new(node.name, node.typed_parameters, body, rescues, node.return_type, visibility: node.visibility, receiver: receiver, is_abstract: node.abstract?, annotations: node.annotations, location: node.location)
                    ...
                    when AST::ClassDefinition
                        AST::ClassDefinition.new(node.name, node.body.map { |n| rewrite_node(n, defaults) }, node.superclass, is_abstract: node.abstract?, annotations: node.annotations, visibility: node.visibility, location: node.location)
                ...
                private def rewrite_method_call(node : AST::MethodCall, defaults : Hash(String, FunctionDefaults)) : AST::Node
                    ...
                    if call_args.any?(&.is_a?(AST::NamedArgument))
                        return rewrite_named_call(node, rewritten_args, call_args, block_arg, info)
                    end
                ...
                private def rewrite_named_call(node : AST::MethodCall, rewritten_args : Array(AST::Node), call_args : Array(AST::Node), block_arg : AST::BlockLiteral?, info : FunctionDefaults) : AST::Node
                    unchanged = AST::MethodCall.new(node.name, rewritten_args, nil, location: node.location)

                    parameter_names = info.parameters.map(&.name)
                    slots = Array(AST::Node?).new(parameter_names.size, nil)
                    next_positional = 0

                    call_args.each do |argument|
                        if argument.is_a?(AST::NamedArgument)
                            named = argument.as(AST::NamedArgument)
                            index = parameter_names.index(named.name)
                            return unchanged unless index
                            return unchanged if slots[index]
                            slots[index] = named.value
                        else
                            while next_positional < slots.size && slots[next_positional]
                                next_positional += 1
                            end
                            return unchanged if next_positional >= slots.size
                            slots[next_positional] = argument
                            next_positional += 1
                        end
                    end

                    expanded = [] of AST::Node

                    slots.each_with_index do |slot, index|
                        if slot
                            expanded << slot
                        else
                            default_node = info.parameters[index].default_value
                        return unchanged unless default_node
                            expanded << default_node
                        end
                    end

                    final_args = block_arg ? (expanded + [block_arg]) : expanded
                    AST::MethodCall.new(node.name, final_args, nil, location: node.location)
                end
            end
        end
    end
