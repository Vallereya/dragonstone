...
Once Dragonstone is installed running files, via the compiler, is pretty straightforward and below is a guide on how. By default, the compiler is with bytecode but if you have Ruby, Crystal, C, or LLVM (IR) toolchains installed, you can compile targets for those as well.
