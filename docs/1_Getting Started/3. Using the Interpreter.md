...
### Selecting a backend:
You can force the interpreter or the bytecode backend.

```bash
    dragonstone run --backend native examples/hello_world.ds   # interpreter only
    dragonstone run --backend core   examples/hello_world.ds   # VM/bytecode (limited support)

    # Or export the flag once for CI:
...
```bash
    ...
    # Backend-aware helper (also works from src/dragonstone/native or src/dragonstone/core)
    # Run via `bash` ./scripts/backend_ci.sh [...] on Windows terminals.
